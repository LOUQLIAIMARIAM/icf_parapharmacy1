...
//
// class anti extends StatefulWidget {
//   const anti({Key? key}) : super(key: key);
//
//   @override
//   State<anti> createState() => _antiState();
//
// }
//
// class _antiState extends State<anti> {
//
//   Cart cart = Cart();
...
//             children: [
//               Image.network(
//                 product.imageUrl, // Replace with the Firebase Storage download URL
//                 height: 150,
...
//       collRef.add({
//         'adresse': _mapLinkController.text,
...
//     CollectionReference payments = FirebaseFirestore.instance.collection('panier');
//
...
// Text("Category: ${categoryDocs?[index]['categoryName']}"),


import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

class AntiClass extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Anti Products'),
      ),
      body: StreamBuilder(
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              var antiCollection = categoryDocs?[index].reference.collection('anti');

              return StreamBuilder(
                stream: antiCollection?.snapshots(),
                builder: (context, snapshot) {
                  if (!snapshot.hasData) {
                    return CircularProgressIndicator();
                  }

                  var antiDocs = snapshot.data?.docs;
                    ...
                    children: [
                      Row(
                        children: [
                          Expanded(
                            child: Column(
                              children: antiDocs?.sublist(0, antiDocs.length ~/ 2).map((antiDoc) {
                                var productData = antiDoc.data();
                                return Card(
                                  child: Column(
                                    children: [
                                      Image.network(
                                        productData?['imageProduit'],
                                        height: 150,
                                        width: double.infinity,
                                        fit: BoxFit.cover,
                                      ),
                                      Text(productData?['nomProduit']),
                                      Text('${productData?['prix']}'),
                                    ],
                                  ),
                                );
                              }).toList() ?? [],
                          ...
                          Expanded(
                            child: Column(
                              children: antiDocs?.sublist(antiDocs.length ~/ 2).map((antiDoc) {
                                var productData = antiDoc.data();
                                return Card(
                                  child: Column(
                                    children: [
